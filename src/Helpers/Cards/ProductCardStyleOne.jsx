...
export default function ProductCardStyleOne({ datas, type = 1, layoutOption = { ratio: 1.33 }, slug }) {
  const available = (datas.cam_product_sale / (datas.cam_product_available + datas.cam_product_sale)) * 100;


  useEffect(() => {
    console.log("datas in ProductCardStyleOne ---- " + JSON.stringify(datas.productId, null, 2));
  }, []);
  return (
    <div
    ...
    >
      ...
      <div className="product-card-details px-[30px] pb-[30px] relative">
        ...
        <a href={`/${slug}/single-product/${datas.productId}`} >
          <p className="title mb-2 text-[18px] font-400 text-qblack leading-[20px] line-clamp-2 hover:text-blue-600" style={{ ontSize: '10px' }}>
            {datas.name}
          </p>
        </a>
        <p className="price">
          {/*<span className="main-price text-qgray line-through font-600 text-[18px]">
            {datas.variants[0].price - 10}
          </span>*/}
          <span className="offer-price text-qred font-500 text-[20px] ml-2">
            {datas.variants[0].price}.-
          </span>
        </p>
      </div>
      {/* quick-access-btns */}
