...
export default function ProductCardStyleOne3Image({ datas, type, layoutOption, colNum }) {
  //const available =  (datas.cam_product_sale /(datas.cam_product_available + datas.cam_product_sale)) * 100;
  const available = 10;

  useEffect(() => {
    //alert(JSON.stringify(layoutOption.ratio));
    // alert("datas in ProductCardStyleOne ---- " + JSON.stringify(datas.imageUrl));
  }, []);
  ...
  return (
    ...
    //style={{ boxShadow: "0px 3px 3px 0px rgba(0, 0, 0, 0.05)", borderRadius: "2px" }}
    >
      <div
        className={`product-card-img w-full`}
        style={{
          width: '100%',
          padding: '0px',
          paddingTop: `${100 * (layoutOption.ratio ?? 1.33)}%`,
          background: `url(${datas.imageUrl}) no-repeat center`,
          backgroundSize: 'cover',
          //background : `url(datas) no-repeat center`,
        }}
      >
      ...
      <div className="product-card-details px-[0px] pb-[30px] relative">
        {/* add to card button */}
        {/*<div className="absolute w-full h-10 px-[30px] left-0 top-40 group-hover:top-[85px] transition-all duration-300 ease-in-out">
          <button type="button" className={type === 3 ? 'blue-btn' : 'yellow-btn'} style={{ borderRadius: "10px" }}>
        ...
        {datas.text1.trim() != "" && (
          <div style={{ marginTop: '10px' }}>
            <p className="title mb-2 text-[18px] font-400 text-qblack leading-[20px] line-clamp-2 " style={{ fontSize: '16px' }}>
              {datas.text1}
            </p>
          </div>)}
        {datas.buttonCaption.trim() != "" && (
          <a href="/single-product">
            <p className="title mb-2 text-[18px] font-400 text-qblack leading-[20px] line-clamp-2 hover:text-blue-600" style={{ fontSize: '16px', textDecoration: 'underline' }}>
              {datas.buttonCaption}
            </p>
          </a>)}
      </div>
